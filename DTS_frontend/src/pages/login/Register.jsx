    ...
    return (
        <section className="flex flex-col md:flex-row h-screen items-center">
            <div className="hidden bg-[#AFCACC] w-full md:w-1/2 xl:w-1/2 h-screen md:flex md:items-center md:justify-center">
                ...
                <h1 className='md:mx-auto md:text-4xl xl:text-6xl mx-auto text-white text-6xl mb-2'>NCU Design Thinking</h1>
                {/* <TypeAnimation
        ...
        <div className=" bg-[#FCFAF8] w-full md:max-w-md lg:max-w-full md:mx-auto md:w-1/2 xl:w-1/2 h-screen px-6 lg:px-20 xl:px-28 flex items-center justify-center rounded-l-lg">
            <div className="w-full h-100">
                ...
                <form className="mt-6">
                    <div>
                        <input type="text" name="username" placeholder="account" onChange={handleChange} className="text-base w-full px-4 py-4 bg-transparent border-b border-gray-500 focus:outline-none" autoFocus required />
                    </div>
                    <div>
                        <input type="password" name="password" placeholder="password" minLength="6" onChange={handleChange} className="text-base w-full px-4 py-4 bg-transparent border-b border-gray-500 focus:outline-none" required />
                    </div>
                    <div>
                        <input type="password" name="confirmPassword" placeholder="confirm password" minLength="6" onChange={handleChange} onBlur={validateInput} className="text-base w-full px-4 py-4 bg-transparent border-b border-gray-500 focus:outline-none" required />
                        {error && <span className='text-xs text-red-600'>{error}</span>}
                    </div>
                    <button type="submit" onClick={handleSubmit} className="w-full block bg-[#8AB3B6] hover:bg-[#578082] focus:bg-[#8AB3B6] text-white font-semibold rounded-lgpx-4 py-3 mt-6 text-base rounded-lg">Creat</button>
                </form>
                <p className="mt-8">
